...
func (a *Syntax) Equal(c cell.T) bool {
	p, ok := c.(*Syntax)
	return ok && p == a
}
...
// Execute sets up the states required to execute the syntax a.
func (a *Syntax) Execute(m *Machine) State {
        return m.ReplaceState(a.State)
}
