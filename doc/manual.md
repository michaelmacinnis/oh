...
cell. A cons cell is a pair of values. A cons cell's values are referred to
as the head and tail. The head of a cons cell is accessed using the `head`
method. The tail of a cons cell is accessed using the `tail` method. The
`cons` command is used to construct a new cons cell.
...
Lists, in oh, are formed by chaining cons cells. The tail of each cons cell
is set to the next cons cell in the list. The tail of the last element in
